...
import { createProject, renderSTL } from './openscad.js';
...
export async function runPass(designDir, options = {}) {
  ...
  // Create a shared project file for both render steps
  let projectFile;
  if (!options.analyzeOnly) {
    try {
      projectFile = await createProject(scadPath);
    } catch (err) {
      report.steps.push({ step: 'create-project', status: 'error', error: err.message });
      report.pass = false;
      return report;
    }
  }
  ...
  try {
    ...
    if (!options.analyzeOnly) {
      try {
        renderResult = await renderSTL(scadPath, stlPath, {}, projectFile);
    ...
    if (!options.analyzeOnly) {
      try {
        const viewResult = await renderViews(scadPath, outputDir, {}, projectFile);
