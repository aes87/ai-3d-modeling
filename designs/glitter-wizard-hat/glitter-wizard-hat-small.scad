// Glitter Wizard Hat Cap — Small (fits 14.5" / 20oz lamps)
...
/* ── Parameters ────────────────────────────────────────────── */
height     =  95.0;   // total height including tip
base_od    =  48.0;   // base outer diameter
...
// Cutout parameters — tuned for smaller circumference
star_d       = 5.5;    // slightly smaller stars
star_inner   = 0.38;
moon_h       = 4.5;    // slightly smaller moons
moon_w       = 3.0;
n_cutouts    = 10;     // fewer shapes on smaller circumference
...
module glitter_wizard_hat_small() {
    difference() {
        cone_body();
        cone_bore();
        cutout_ring();
...
glitter_wizard_hat_small();
report_dimensions(base_od, base_od, height, "glitterWizardHatSmall");
