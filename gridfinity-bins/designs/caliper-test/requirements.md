...
- **Purpose**: Houses the caliper's display body. Forms the wide lower portion of the two-stage pocket. The caliper weight rests on the ledge transition above.
- **Critical dimensions**: 70 mm wide (X) × 18 mm deep (Y); height from bin floor = 64 mm (Z = 7.2 mm to Z = 71.2 mm). Pocket is centered in the bin's XY.
- **Mating interfaces**: Caliper display body (68 × 16 mm nominal, ±2 mm). Clearance fit: +1 mm each side (+2 mm total per axis). Tighter fit than v1 for snug caliper seating.

### Feature 5: Beam Slot (upper pocket)

- **Purpose**: Guides the beam above the display body cavity. The narrowing from display cavity to beam slot creates the L-shaped support ledge. The slot continues from the display cavity top through the stacking lip.
- **Critical dimensions**: 18 mm wide (X) × 7 mm deep (Y); height from Z = 71.2 mm to Z = 88.4 mm (12.8 mm through body + 4.4 mm through lip). Positioned at the min-X, min-Y corner of the display cavity footprint (X: −35 to −17, Y: −9 to −2).
- **Beam positioning note**: The beam runs along one edge of the display body. The beam slot is at the min-X, min-Y corner of the display cavity, matching the actual caliper cross-section where the beam sits at one corner of the display body, not centered.
- **Mating interfaces**: Caliper beam (16 × 5 mm nominal, ±2 mm). Clearance fit: +1 mm each side.

### Feature 6: Pocket Transition Ledge

- **Purpose**: The L-shaped horizontal surface where the display body cavity narrows to the beam slot. The caliper's display body upper rim rests on this ledge, supporting the full weight of the caliper. This is the primary load-bearing surface.
- **Critical dimensions**: Ledge width in X = (70 − 18) / 2 = 26 mm on each side. Ledge width in Y = (18 − 7) / 2 = 5.5 mm on each side. Ledge is at Z = 71.2 mm elevation. The 18×7 mm beam slot passes through at the min-X, min-Y corner; remaining area is solid ledge supporting caliper on 3 sides.
- **Printability**: The ledge is a horizontal surface at Z = 71.2 mm. It is fully supported from below by solid pocket walls. No bridge or overhang issue.

### Feature 7: Finger Relief — REMOVED (v2)

- In v2, the finger relief chamfer was removed. The caliper beam extends above the bin rim and the user grabs the beam directly. No finger access into the pocket is needed.
...
The jaw length (40 mm from beam edge) is accommodated within the display cavity width of 70 mm: jaw extends 40 mm from beam edge; beam slot is at the min-X, min-Y corner, so 26 mm of cavity width is available on each side of the beam slot in X — more than enough for the 40 mm jaw. The jaw extends in the Y direction (perpendicular to beam axis). With 18 mm Y depth in the cavity, the jaw (which is thin, ~5 mm thick) fits with clearance.
